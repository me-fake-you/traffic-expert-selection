...
CONFORMAL_V33_SCHEMA_VERSION = "1.0"
...
def fit_regime_conformal_v33(
    ...
    *,
    ...
    if regime not in {"short", "long"}:
        raise ValueError(f"unsupported conformal_v3_3 regime: {regime}")
    if alpha != 0.01 or hard_p_value != 0.01:
        raise ValueError(
            "conformal_v3_3 freezes alpha and hard p-value at 0.01"
        )
    references_input = np.asarray(reference_embeddings, dtype=np.float32)
    ...
    if set(np.unique(reference_labels)) != {0, 1}:
        raise ValueError("conformal_v3_3 reference requires both classes")
    if set(np.unique(calibration_labels)) != {0, 1}:
        raise ValueError("conformal_v3_3 calibration requires both classes")
    ...
    metadata = {
        "schema_version": CONFORMAL_V33_SCHEMA_VERSION,
...
class RegimeConformalGateV33:
    name = "RegimeConformalGateV33"
    version = "3.3"
    ...
    def __init__(self, artifact_dir: str | Path, *, regime: str) -> None:
        ...
        if not metadata_path.exists() or not artifact_path.exists():
            raise ValueError(
                f"incomplete conformal_v3_3 {regime} artifact: {root}"
            )
        self.metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
        if (
            self.metadata.get("schema_version")
            != CONFORMAL_V33_SCHEMA_VERSION
            or self.metadata.get("policy") != "conformal_v3_3"
            or self.metadata.get("regime") != regime
        ):
            raise ValueError("conformal_v3_3 metadata mismatch")
        if _sha256(artifact_path) != self.metadata.get("artifact_sha256"):
            raise ValueError("conformal_v3_3 artifact checksum mismatch")
